...
vertx.createNetServer().connectHandler({ socket: NetSocket =>
  socket.dataHandler({ x: Buffer =>
    container.logger.info("Buffer: " + x.toString())
    RecordParser.newDelimited("\n", { frame: Buffer =>
      ...
      if (line.startsWith("subscribe")) {
        ...
        val set = vertx.sharedData.getSet[String](parts(1))
        ...
        container.logger.info("Publish to topic '" + parts(1) + "'")
        val actorIDs = vertx.sharedData.getSet[String](parts(1))
